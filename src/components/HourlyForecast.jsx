...
const getWeatherIcon = (condition) => {
  switch (condition.toLowerCase()) {
    case 'clear': return <WiDaySunny className="text-yellow-400 text-xl sm:text-2xl" />;
    case 'rain': return <WiRain className="text-blue-400 text-xl sm:text-2xl" />;
    case 'clouds': return <WiCloudy className="text-gray-400 text-xl sm:text-2xl" />;
    case 'snow': return <WiSnow className="text-blue-200 text-xl sm:text-2xl" />;
    case 'thunderstorm': return <WiThunderstorm className="text-purple-500 text-xl sm:text-2xl" />;
    default: return <WiDaySunny className="text-yellow-400 text-xl sm:text-2xl" />;
  }
};
...
const HourlyForecast = ({ forecastData }) => {
  ...
  return (
    ...
    ">
      ...
      <div className="flex overflow-x-auto pb-3 sm:pb-4 -mx-2 px-2">
        {next24Hours.map((hour, index) => (
          <div
            key={index}
            className="flex flex-col items-center px-2 sm:px-3 flex-shrink-0 min-w-[56px] sm:min-w-[72px]"
          >
